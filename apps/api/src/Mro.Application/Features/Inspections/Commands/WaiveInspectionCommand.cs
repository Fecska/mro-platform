...
public sealed class WaiveInspectionCommand : IRequest<Result<Unit>>
{
    public required Guid InspectionId { get; init; }
    public required string Reason { get; init; }
}

public sealed class WaiveInspectionCommandHandler(
    IInspectionRepository inspections,
    ICurrentUserService currentUser)
    : IRequestHandler<WaiveInspectionCommand, Result<Unit>>
{
    public async Task<Result<Unit>> Handle(WaiveInspectionCommand request, CancellationToken ct)
    {
        ...
        var result = inspection.Waive(request.Reason, currentUser.UserId!.Value);
        if (result.IsFailure)
