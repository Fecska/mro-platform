...
public sealed class IssueWorkOrderCommand : IRequest<Result<Unit>>
{
    public required Guid WorkOrderId { get; init; }
}

public sealed class IssueWorkOrderCommandHandler(
    IWorkOrderRepository workOrders,
    ICurrentUserService currentUser)
    : IRequestHandler<IssueWorkOrderCommand, Result<Unit>>
{
    public async Task<Result<Unit>> Handle(IssueWorkOrderCommand request, CancellationToken ct)
    {
        ...
        var domainResult = wo.Issue(currentUser.UserId!.Value);
